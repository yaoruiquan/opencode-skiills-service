/**
 * Deterministic adapter for phase2-ncc-report.
 *
...
const SKILL_NAME = "phase2-ncc-report";
...
async function run(job, body, mode, context = {}) {
  ...
  // NCC prepare_form_context supports --input-path / --docx-path or a positional DAS-ID.
  if (target.toLowerCase().endsWith(".docx")) {
    args.push("--docx-path", target);
  } else {
    args.push("--input-path", target);
  }
  if (serviceConfig.prefer_source) {
    args.push("--prefer-source", serviceConfig.prefer_source);
  }
